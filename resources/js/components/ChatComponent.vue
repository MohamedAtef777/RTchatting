<template>
    <div class="flex flex-col h-[500px]">
        <div class="flex items-center">
            <h1 class="text-lag font-semibold mr-2">{{ user.name}}</h1>
        ...
        <div class="border-t pt-4">
            <form @submit.prevent="sendMessage">
                <div class="flex items-center">
                    ...
                    <button type="submit" class="ml-2 bg-indigo-500 text-black p-3 rounded-lg hover:bg-indigo-600 transition duration-300 flex items-center justify-center">
                        <i class="fas fa-paper-plane">
                            <img width="30" height="20" src="https://img.icons8.com/color/48/sent--v1.png" alt="sent--v1"/>
                        </i>
                    </button>
                </div>
            </form>
    ...
    <small v-if="isUserTyping" class="text-gray-600 mt-5">{{ user.name }} is typing...</small>
</template>
...
const sendMessage = async () => {
    if (newMessage.value.trim() === '') return; // Only send if message is not empty
    try {
...
onMounted(() => {
    ...
    // Presence Channel for user online status
